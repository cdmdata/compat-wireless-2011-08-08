...
static void wl1271_parse_fw_ver(struct wl1271 *wl)
{
	...
	if (ret != 5) {
		wl1271_error("fw version incorrect value");
...
static int wl1271_boot_upload_firmware(struct wl1271 *wl)
{
	...
	wl1271_error( "firmware chunks to be uploaded: %u", chunks);
...
static int wl1271_boot_soft_reset(struct wl1271 *wl)
{
	...
	while (1) {
		...
		wl1271_error( "soft reset bootdata 0x%x", boot_data);
...
printk (KERN_ERR "%s\n", __func__);
	...
	wl1271_error( "ACX_EEPROMLESS_IND_REG");

	wl1271_write32(wl, ACX_EEPROMLESS_IND_REG, ACX_EEPROMLESS_IND_REG);

	tmp = wl1271_read32(wl, CHIP_ID_B);

	wl1271_error( "chip id 0x%x", tmp);
